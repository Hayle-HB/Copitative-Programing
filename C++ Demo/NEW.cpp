...
// Function to load accounts from a file into a vector
void loadAccounts(std::vector<BankAccount>& accounts) {
    ...
    while (std::getline(inFile, line)) {
        ...
        if (tokens.size() >= 3) {
            ...
            account.accountHolderName = tokens[1];
            account.balance = std::stod(tokens[2]);
...
// Function to save accounts from a vector to a file
...
void createAccount(std::vector<BankAccount>& accounts) {
    ...
    accounts.push_back(account);
    saveAccounts(accounts); // Save the new account to the file
...
void depositMoney(std::vector<BankAccount>& accounts) {
    ...
    for (BankAccount& account : accounts) {
        if (account.accountNumber == accountNumber) {
            ...
            saveAccounts(accounts); // Update the account information in the file
...
void displayTransactionHistory(const std::vector<BankAccount>& accounts, const std::string& accountNumber) {
    for (const BankAccount& account : accounts) {
        if (account.accountNumber == accountNumber) {
            std::cout << "\033[1;32mTransaction history for account " << account.accountNumber << " (Holder: " << account.accountHolderName << "):\033[0m" << std::endl;
            std::cout << "\033[1;32mBalance: " << account.balance << "\033[0m" << std::endl;
            return;
        }
    }
...
int main() {
    ...
    loadAccounts(accounts); // Load existing accounts from the file
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 6:
                ...
                displayTransactionHistory(accounts, accountNumber);
                break;
            case 7:
